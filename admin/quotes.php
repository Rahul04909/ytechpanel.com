<?php
/**
 * YTech Panels — Admin Quotes Management
 */
include './header.php';
$db = getDB();

if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_quote_status') {
    $id = (int)($_POST['id'] ?? 0);
    $status = (int)($_POST['status'] ?? 0);
    if ($id > 0) {
        $stmt = $db->prepare("UPDATE quotes SET status = ? WHERE id = ?");
        $stmt->execute([$status, $id]);
        $successMsg = 'Quote status updated.';
    }
}
...
<div class="admin-card">
    <div class="admin-card-header">
        <h5><i class="fas fa-file-invoice-dollar" style="color:#2271b1;"></i> Quote Requests <span class="badge bg-primary ms-2"><?= count($quotes) ?></span></h5>
    </div>
    <?php if (empty($quotes)): ?>
    ...
    <div style="overflow-x:auto;">
        <table class="data-table">
            <thead><tr><th>Date</th><th>Name</th><th>Email</th><th>Phone</th><th>Company</th><th>Product</th><th>Qty</th><th>Message</th><th>Status</th><th>Action</th></tr></thead>
            <tbody>
                <?php foreach ($quotes as $q): ?>
                <tr>
                    <td style="white-space:nowrap;font-size:12px;color:#64748b;"><?= date('d M Y<br>h:i A', strtotime($q['created_at'])) ?></td>
                    <td><strong><?= htmlspecialchars($q['name']) ?></strong></td>
                    <td><a href="mailto:<?= htmlspecialchars($q['email']) ?>" style="color:#2271b1;"><?= htmlspecialchars($q['email']) ?></a></td>
                    <td><?= !empty($q['phone']) ? htmlspecialchars($q['phone']) : '—' ?></td>
                    <td><?= !empty($q['company']) ? htmlspecialchars($q['company']) : '—' ?></td>
                    <td><?= !empty($q['product_interest']) ? htmlspecialchars($q['product_interest']) : '—' ?></td>
                    <td><?= !empty($q['quantity']) ? htmlspecialchars($q['quantity']) : '—' ?></td>
                    <td style="max-width:160px;font-size:13px;color:#64748b;"><?= htmlspecialchars(substr($q['message'], 0, 80)) ?><?= strlen($q['message']) > 80 ? '...' : '' ?></td>
                    <td><span class="badge-status <?= $q['status'] == 0 ? 'badge-new' : ($q['status'] == 1 ? 'badge-contacted' : 'badge-quoted') ?>"><?= $q['status'] == 0 ? 'New' : ($q['status'] == 1 ? 'Contacted' : 'Quoted') ?></span></td>
                    <td>
                        <form method="POST" style="display:flex;gap:4px;">
                            ...
                            <select name="status" class="status-select">
                                ...
                                <option value="1" <?= $q['status'] == 1 ? 'selected' : '' ?>>Contacted</option>
                                <option value="2" <?= $q['status'] == 2 ? 'selected' : '' ?>>Quoted</option>
                            </select>
